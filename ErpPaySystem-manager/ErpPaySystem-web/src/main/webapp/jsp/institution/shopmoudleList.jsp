...
<script type="text/javascript">
	...
	$("#search").click(function() {
		var shopmoudlename = $("#shopmoudlename").val();
		param = {
			shopmoudlename : shopmoudlename
		}
		tableIns.reload({
			where : param,
			page : {
		...
		table.on('tool(test)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			...
			if (layEvent === 'detail') { //查看
				...
				layer.open({
					...
					closeBtn : 2,
					type : 2,
					content : '${ctx}/jsp/institution/shopmoudleList.jsp',
					area : [ '1000px', '500px' ]
				});
			}
		});
		table.on('toolbar(test)', function(obj) {
			...
			case 'add':
				layer.open({
					...
					closeBtn : 2,
					type : 2,
					content : '${ctx}/jsp/institution/shopmoudleAdd.jsp',
					area : [ '700px', '500px' ]
				});
				break;
			case 'delete':
